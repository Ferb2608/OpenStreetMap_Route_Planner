...
// Function compare value of 2 nodes for sort open_list according to the sum of the h value and g value
bool Compare(const RouteModel::Node *node_a, const RouteModel::Node *node_b)
{
    float node_a_f = node_a->h_value + node_a->g_value;
    float node_b_f = node_b->h_value + node_b->g_value;
    return node_a_f > node_b_f;
}

RouteModel::Node *RoutePlanner::NextNode()
{
    std::sort(RoutePlanner::open_list.begin(), RoutePlanner::open_list.end(), Compare);
    RouteModel::Node *lowest_node = RoutePlanner::open_list.back();
